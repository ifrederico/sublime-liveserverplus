...
from typing import Any, Callable, Dict, Iterable as TypingIterable, List, Literal, Optional, Tuple, Type, TypedDict, Union
...
_link_patterns = TypingIterable[Tuple[re.Pattern, Union[str, Callable[[re.Match], str]]]]
...
class MarkdownFileLinks(LinkProcessor):
    ...
    def parse_inline_anchor_or_image(self, text: str, _link_text: str, start_idx: int):
        ...
        if not result or not result[1] or not result[1].endswith('.md'):
            ...
            return
        url = _remove_suffix(result[1], '.md') + '.html'
    ...
    def run(self, text: str):
        if Stage.LINKS > self.md.order > Stage.LINK_DEFS and self.options.get('link_defs', True):
            ...
            for key, url in self.md.urls.items():
                if url.endswith('.md'):
                    self.md.urls[key] = _remove_suffix(url, '.md') + '.html'
